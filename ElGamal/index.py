...
while 1:
    p = input("Nhập p: ")
    if not p.isdigit():
        print("p phải là 1 chữ số! Vui lòng nhập lại")
        continue
    p = int(p)
...
while 1:
    ...
    if not x1.isdigit():
        print(f"Khóa riêng tư của cả user 1 không hợp lệ, Vui lòng nhập lại!")
        continue
    x1 = int(x1)
    if x1 > p - 1:
        print(f"Khóa riêng tư của cả user 1 phải nhỏ hơn {p - 1}, Vui lòng nhập lại!")
        continue
    
    x2 = input("Enter The Private Key Of User 2: ")
    
    if not x2.isdigit():
        print(f"Khóa riêng tư của cả user 2 không hợp lệ, Vui lòng nhập lại!")
        continue
    x2 = int(x2)
    if x2 > p - 1:
        print(f"Khóa riêng tư của cả user 2 phải nhỏ hơn {p - 1}, Vui lòng nhập lại!")
        continue
...
while 1:
    P = input("Nhập P(phần tử rõ) : ")
    
    if not P.isdigit():
        print("Phần tử rõ không hợp lệ. Vui lòng nhập lại.")
        continue
    P = int(P)
    if P > p:
